...
const Explore: React.FC<ConnectedProps<typeof connector>> = ({
  data,
  categories,
}) => {
  ...
  const filteredData = React.useMemo(() => {
    return data
      .filter(
        c =>
          selectedCategory === undefined || c.categoryId === selectedCategory,
      )
      .sort((a, b) =>
        selectedTab === 0 ? 0 : (b.traineeCount ?? 0) - (a.traineeCount ?? 0),
      );
  ...
  const renderItem: ListRenderItem<Course> = React.useCallback(({ item }) => {
    return (
      <CourseItem
        item={item}
        category={categories.find(v => v.id === item.categoryId)!.name}
        onClick={() =>
