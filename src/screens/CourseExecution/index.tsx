...
const ContentSwitcher = ({ task, detail }: { task: Task; detail: TaskDetail }) => {
    switch (task.type) {
        case TaskType.Content:
            return (
                React.useMemo(() => <ContentTask taskDetail={detail} />, [detail.id])
            );
        case TaskType.Survey:
        case TaskType.Quiz:
        case TaskType.EditableForm:
            return (
                React.useMemo(() => <SurveyTask taskDetail={detail} />, [detail.id])
            );
        default:
...
const CourseExecution = (_: Props) => {
    ...
    return <ScrollView style={GlobalStyles.screenContainer} contentContainerStyle={GlobalStyles.screenContainer}>
        ...
        {task && taskDetail && < ContentSwitcher task={task} detail={taskDetail} />}
    </ScrollView>;
};
